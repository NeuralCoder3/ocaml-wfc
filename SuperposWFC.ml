...
let data_predicate test f graph =
    let nodes = graph.nodes in
    test (fun n -> f n.data) nodes


let impossible graph =
data_predicate exists (fun data -> data = []) graph

let finished graph =
data_predicate for_all (fun data -> 
    match data with 
    | [_] -> true
    | _ -> false
) graph
